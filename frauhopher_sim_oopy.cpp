    ...
    , ui(new Ui::frauhopher_sim)
{
    ...
    for (int xIndex=0; xIndex<nx; ++xIndex)
    {
        ...
        {
            colorMap->data()->cellToCoord(xIndex, yIndex, &x0, &);
//            double r = 3*qSqrt(x*x+y*y)+1e-2;
//            z = 2*x*(qCos(r+2)/r-qSin(r+2)/r); // the B field strength of dipole radiation (modulo physical constants)
            z = I0*qPow(qSin(x0*wavelength*a/distance),2)*qPow(qSin(*wavelength*b/distance),2)/(x0*x0**);
//            z = I0*qPow(qSin(x0*wavelength*a/distance),2)*qPow(qSin(*wavelength*b/distance),2);
